...
;(function($) {
  ...
  var toggleBox = function(event) {
    ...
    if ((isTargetOrChild || isBarOrChild) && $("#preview iframe").contents()[0].getSelection().toString().length > 0 && !$.popline.current.keepSlientWhenBlankSelected()) {
  ...
  var targetEvent = {
    ...
    keyup: function(event) {
      ...
      if (!isIMEMode && $("#preview iframe").contents()[0].getSelection().toString().length > 0 && !popline.keepSlientWhenBlankSelected()) {
    ...
    keydown: function(event) {
      $.popline.current = $(this).data("popline");
      var rects = $("#preview iframe").contents()[0].getSelection().getRangeAt(0).getClientRects();
      if (rects.length > 0) {
    ...
    var positions = {
      "fixed": {
        mouseup: function(event) {
          var rect = $("#preview iframe").contents()[0].getSelection().getRangeAt(0).getBoundingClientRect();
  ...
  $.extend($.popline, {
    ...
    prototype: {
      ...
      keepSlientWhenBlankSelected: function() {
        if (this.settings.keepSlientWhenBlankSelected && $.trim($("#preview iframe").contents()[0].getSelection().toString()) === ""){
          return true;
        }else {
          return false;
        }
    ...
    boundingRect: function(rects) {
      if ($.popline.utils.isNull(rects)) {
        rects = $("#preview iframe").contents()[0].getSelection().getRangeAt(0).getClientRects();
      }
      return {
